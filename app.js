...
async function downloadSppdPdf(json){
  ...
  const docDate=x.created_at ? new Date(x.created_at) : new Date();
  const locDate=`${x.lokasi_surat||'-'}, ${docDate.toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'})}`;
...
function pickSppd(i){
  ...
  $('pBank').value=s.bank||'';$('pRek').value=s.no_rekening||'';$('pPengaju').value=s.nama;$('pPengajuJab').value=s.jabatan||'';
  $('pTtd2Jenis').value='MENGETAHUI';$('pTtd2Nama').value='';$('pTtd2Jab').value='';$('pTtd2Label').value='';
  $('pTtd3Jenis').value='';$('pTtd3Nama').value='';$('pTtd3Jab').value='';$('pTtd3Label').value='';
  $('pTtd4Jenis').value='';$('pTtd4Nama').value='';$('pTtd4Jab').value='';$('pTtd4Label').value='';
  [2,3,4].forEach(togglePjpdSignatureOther);
  recalc();
}
function addExtra(){
...
function togglePjpdSignatureOther(n){
  const jenis=val(`pTtd${n}Jenis`);
  $(`pTtd${n}LabelWrap`).classList.toggle('hidden',jenis!=='LAINNYA');
}
function pjpdSignatureLabel(jenis,customLabel=''){
  const labels={
  ...
  if(jenis==='LAINNYA')return customLabel.trim();
  return labels[jenis]||'';
}
function readPjpdSignature(n){
  const jenis=val(`pTtd${n}Jenis`);
  const nama=val(`pTtd${n}Nama`).trim();
  const jabatan=val(`pTtd${n}Jab`).trim();
  const custom=val(`pTtd${n}Label`).trim();
  return {
    jenis,
    label:pjpdSignatureLabel(jenis,custom),
    nama,
    jabatan
  };
}
...
async function submitPjpd(){
  ...
  const ttd2=readPjpdSignature(2);
  const ttd3=readPjpdSignature(3);
  const ttd4=readPjpdSignature(4);

  if(!ttd2.jenis)return alert('Tanda Tangan 2 wajib dipilih.');
  if(!ttd2.label)return alert('Label Tanda Tangan 2 wajib diisi.');
  if(!ttd2.nama||!ttd2.jabatan)return alert('Nama dan Jabatan Tanda Tangan 2 wajib diisi.');

  for(const [n,sig] of [[3,ttd3],[4,ttd4]]){
    const adaIsi=sig.jenis||sig.nama||sig.jabatan||sig.label;
    if(adaIsi){
      if(!sig.jenis)return alert(`Jenis Tanda Tangan ${n} wajib dipilih jika posisi ini digunakan.`);
      if(!sig.label)return alert(`Label Tanda Tangan ${n} wajib diisi.`);
      if(!sig.nama||!sig.jabatan)return alert(`Nama dan Jabatan Tanda Tangan ${n} wajib lengkap.`);
    }
  }

  const details=getDetails();if(!details.length)return alert('Rincian biaya belum diisi.');
  const payload={
    sppd_id:selectedSppd.id,
    details,
    ttd2_jenis:ttd2.jenis,ttd2_label:ttd2.label,ttd2_nama:ttd2.nama,ttd2_jabatan:ttd2.jabatan,
    ttd3_jenis:ttd3.jenis,ttd3_label:ttd3.label,ttd3_nama:ttd3.nama,ttd3_jabatan:ttd3.jabatan,
    ttd4_jenis:ttd4.jenis,ttd4_label:ttd4.label,ttd4_nama:ttd4.nama,ttd4_jabatan:ttd4.jabatan
  };

  loading(true);
  const {data,error}=await sb.rpc('create_pjpd',{p:payload});
  loading(false);
  if(error)return alert(error.message);

  const pdfData={
    ...payload,
    no_pjpd:data.no_pjpd,
    total:data.total_pjpd,
    selisih:data.kurang_lebih_bayar,
    s:selectedSppd
  };
...
async function downloadPjpdPdf(json){
  ...
  doc.setFontSize(8.5);
  let y=34;
  const pjpdDocDate=x.created_at ? new Date(x.created_at) : new Date();
  textPair(doc,'Tanggal PJPD',pjpdDocDate.toLocaleDateString('id-ID',{day:'2-digit',month:'long',year:'numeric'}),12,58,y,false);
  ...
  const signers=[
    ...
    {title:x.ttd2_label||'',nama:x.ttd2_nama||'',jabatan:x.ttd2_jabatan||''},
    x.ttd3_jenis ? {title:x.ttd3_label||'',nama:x.ttd3_nama||'',jabatan:x.ttd3_jabatan||''} : null,
    x.ttd4_jenis ? {title:x.ttd4_label||'',nama:x.ttd4_nama||'',jabatan:x.ttd4_jabatan||''} : null
  ].filter(Boolean);

  const y0=signY+4,h=38,x0=11,w=275,cw=w/signers.length;
  for(let i=0;i<signers.length;i++){
    const sg=signers[i];
    doc.rect(x0+i*cw,y0,cw,h);
    doc.setFont('helvetica','bold');
    doc.text(pdfSafe(sg.title),x0+i*cw+cw/2,y0+6,{align:'center',maxWidth:cw-5});
    if(sg.nama){
      doc.setFont('helvetica','bold');
      doc.text(pdfSafe(sg.nama),x0+i*cw+cw/2,y0+28,{align:'center',maxWidth:cw-5});
    }
    doc.setFont('helvetica','normal');
    if(sg.jabatan)doc.text(pdfSafe(sg.jabatan),x0+i*cw+cw/2,y0+34,{align:'center',maxWidth:cw-5});
  }
...
window.togglePjpdSignatureOther=togglePjpdSignatureOther;
