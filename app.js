...
function pickEmployee(i){
  const e=$('sEmpResults')._data[i];selectedEmployee=e;$('sNamaQ').value=e.nama;$('sNip').value=e.nip||'';$('sJabatan').value=e.jabatan||'';$('sLokasi').value=e.lokasi_kerja||'';$('sMor').value=e.project_mor||'';$('sWbs').value=e.kode_wbs||'';$('sEmpResults').classList.add('hidden');checkDuplicateNow()
}
...
async function submitSppd(){
  ...
  if(!val('sDitNama')||!val('sDitJab'))return alert('Ditugaskan Oleh wajib diisi.');
...
function recalc(){const total=getDetails().reduce((a,x)=>a+x.jumlah,0),panjar=num('pPanjar'),sel=total-panjar;$('sumTotal').textContent=rp(total);$('sumPanjar').textContent=rp(panjar);$('sumSelisih').textContent=rp(sel);$('sumTerbilang').textContent=terbilangKL(sel)}
...
async function submitPjpd(){
  if(!selectedSppd)return alert('Pilih SPPD.');if(!val('pBank')||!val('pRek'))return alert('Bank/No Rekening belum ada di master karyawan.');
  if(!val('pSetNama')||!val('pSetJab'))return alert('Menyetujui (User) wajib diisi.');
  if((val('pKetNama')||val('pKetJab'))&&(!val('pKetNama')||!val('pKetJab')))return alert('Diketahui (User) harus diisi lengkap atau dikosongkan.');
  const details=getDetails();if(!details.length)return alert('Rincian biaya belum diisi.');
  loading(true);const payload={sppd_id:selectedSppd.id,panjar:num('pPanjar'),details,menyetujui_nama:val('pSetNama'),menyetujui_jabatan:val('pSetJab'),diketahui_nama:val('pKetNama'),diketahui_jabatan:val('pKetJab')};
  const {data,error}=await sb.rpc('create_pjpd',{p:payload});loading(false);if(error)return alert(error.message);
  $('pjpdSaved').innerHTML=`<div class="box"><b>PJPD berhasil dibuat</b><br>${esc(data.no_pjpd)}<p><button onclick='downloadPjpdPdf(${JSON.stringify(JSON.stringify({...payload,no_pjpd:data.no_pjpd,total:data.total_pjpd,selisih:data.kurang_lebih_bayar,s:selectedSppd}))})'>Download PDF PJPD</button></p></div>`;
}
