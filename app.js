...
async function downloadPjpdPdf(json){
  ...
  totalRow('Panjar / Uang Muka',rp(x.panjar));
  totalRow('Kurang / (Lebih) Bayar',rp(x.selisih));

  doc.rect(tx,cy,tw,8);
  const split=tx+105;
  doc.line(split,cy,split,cy+8);
  doc.setFont('helvetica','bold');doc.text('Terbilang Kurang / (Lebih) Bayar',tx+2,cy+5.2);
  doc.setFont('helvetica','italic');doc.text(terbilangKL(x.selisih),split+3,cy+5.2);
  cy+=8;
