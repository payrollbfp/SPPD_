...
function openDashboardGate(){
  ...
  $('dashboardGateError').textContent='';
  $('dashboardGateModal').classList.remove('hidden');
  setTimeout(()=>$('dashboardAccessPassword')?.focus(),100);
}
function closeDashboardGate(){
  $('dashboardGateModal').classList.add('hidden');
  $('dashboardAccessPassword').value='';
  $('dashboardGateError').textContent='';
}
async function verifyDashboardGate(){
  const password=$('dashboardAccessPassword').value;
  if(!password){
    $('dashboardGateError').textContent='Password akses Dashboard wajib diisi.';
    return;
  }
  $('dashboardGateError').textContent='Memverifikasi...';
  const {data,error}=await sb.rpc('verify_dashboard_access',{p_password:password});
  if(error){
    $('dashboardGateError').textContent=error.message;
    return;
  }
  if(!data?.ok){
    $('dashboardGateError').textContent='Password akses Dashboard salah.';
    return;
  }
  sessionStorage.setItem('bfp_dashboard_gate','ok');
  closeDashboardGate();
  await openDashboard();
}
async function changeDashboardAccessPassword(){
  const oldPassword=$('setOldDashboard').value;
  const newPassword=$('setNewDashboard').value;
  const confirmPassword=$('setConfirmDashboard').value;
  const msg=$('setMsgDashboard');
  msg.textContent='';
  if(!oldPassword||!newPassword||!confirmPassword){
    msg.textContent='Semua kolom wajib diisi.'; return;
  }
  if(newPassword!==confirmPassword){
    msg.textContent='Konfirmasi password baru tidak sama.'; return;
  }
  if(newPassword.length<6){
    msg.textContent='Password baru minimal 6 karakter.'; return;
  }
  msg.textContent='Menyimpan perubahan...';
  const {error}=await sb.rpc('change_dashboard_access_password',{
    p_old_password:oldPassword,
    p_new_password:newPassword
  });
  if(error){msg.textContent=error.message; return;}
  $('setOldDashboard').value=''; $('setNewDashboard').value=''; $('setConfirmDashboard').value='';
  msg.textContent='Password akses Dashboard berhasil diubah.';
}
...
function openFinalizeSettings(){
  ['setMsgDashboard','setMsgSppd','setMsgTicket','setMsgBilling'].forEach(id=>{if($(id))$(id).textContent='';});
  ['setOldDashboard','setNewDashboard','setConfirmDashboard','setOldSppd','setNewSppd','setConfirmSppd','setOldTicket','setNewTicket','setConfirmTicket','setOldBilling','setNewBilling','setConfirmBilling']
...
async function loadSppdDashboard(){
  ...
  $('dSppdBody').innerHTML=dashSppdRows.map((x,i)=>{
    ...
    return `<tr>
      ...
      <td class="nowrap">
        <button class="btn-small gray" onclick="redownloadSppdPdf('${x.id}')">PDF SPPD</button>
        ${p?`<button class="btn-small" onclick="redownloadPjpdPdf('${p.id}')">PDF PJPD</button>`:''}
      </td>
...
// ---------- DOWNLOAD ULANG PDF SPPD / PJPD ----------
async function redownloadSppdPdf(sppdId){
  loading(true);
  try{
    const {data:s,error:se}=await sb.from('sppd').select('*').eq('id',sppdId).single();
    if(se)throw se;

    let pic={};
    if(s.mengetahui1_tipe==='AREA_PROJECT_OFFICER'||s.mengetahui2_tipe==='AREA_PROJECT_OFFICER'){
      const {data:pd}=await sb.from('project_officers')
        .select('nama,jabatan')
        .eq('project_mor',s.project_mor)
        .eq('aktif',true)
        .limit(1)
        .maybeSingle();
      pic=pd||{};
    ...
    const employee={
      ...
      nama:s.nama||'',
      jabatan:s.jabatan||'',
      lokasi_kerja:s.lokasi_kerja||'',
      project_mor:s.project_mor||'',
      mor_code:s.mor_code||'',
      kode_wbs:s.kode_wbs||''
    };
    ...
    const pdfData={
      no_sppd:s.no_sppd,
      employee,
      lokasi_surat:s.lokasi_surat||'',
      kendaraan:s.kendaraan||'',
      tanggal_mulai:s.tanggal_mulai,
      tanggal_selesai:s.tanggal_selesai,
      tujuan:s.tujuan||'',
      agenda:s.agenda||'',
      ditugaskan_nama:s.ditugaskan_nama||'',
      ditugaskan_jabatan:s.ditugaskan_jabatan||'',
      mengetahui1_tipe:s.mengetahui1_tipe||'',
      mengetahui1_nama:s.mengetahui1_nama||'',
      mengetahui1_jabatan:s.mengetahui1_jabatan||'',
      mengetahui2_tipe:s.mengetahui2_tipe||'',
      mengetahui2_nama:s.mengetahui2_nama||'',
      mengetahui2_jabatan:s.mengetahui2_jabatan||'',
      pic_nama:pic.nama||s.project_officer_nama||'',
      pic_jabatan:pic.jabatan||s.project_officer_jabatan||'Area Project Officer',
      created_at:s.created_at
    };

    await downloadSppdPdf(JSON.stringify(pdfData));
  }catch(e){
    alert('Gagal membuat ulang PDF SPPD: '+(e?.message||e));
  }finally{
    loading(false);
  }
}

async function redownloadPjpdPdf(pjpdId){
  loading(true);
  try{
    const {data:p,error:pe}=await sb.from('pjpd').select('*').eq('id',pjpdId).single();
    if(pe)throw pe;

    const [{data:s,error:se},{data:details,error:de}]=await Promise.all([
      sb.from('sppd').select('*').eq('id',p.sppd_id).single(),
      sb.from('pjpd_details').select('komponen,qty,satuan,tarif,jumlah,urutan').eq('pjpd_id',pjpdId).order('urutan')
    ]);
    if(se)throw se;
    if(de)throw de;
    ...
    const sppdForPdf={
      ...s,
      bank:p.bank||s.bank||'',
      no_rekening:p.no_rekening||s.no_rekening||'',
      nama:p.nama||s.nama||'',
      jabatan:p.jabatan||s.jabatan||'',
      lokasi_kerja:p.lokasi_kerja||s.lokasi_kerja||'',
      project_mor:p.project_mor||s.project_mor||'',
      kode_wbs:p.kode_wbs||s.kode_wbs||''
    };

    // V13 flexible signatures.
    let sig={
      ttd2_jenis:p.ttd2_jenis||'',
      ttd2_label:p.ttd2_label||'',
      ttd2_nama:p.ttd2_nama||'',
      ttd2_jabatan:p.ttd2_jabatan||'',
      ttd3_jenis:p.ttd3_jenis||'',
      ttd3_label:p.ttd3_label||'',
      ttd3_nama:p.ttd3_nama||'',
      ttd3_jabatan:p.ttd3_jabatan||'',
      ttd4_jenis:p.ttd4_jenis||'',
      ttd4_label:p.ttd4_label||'',
      ttd4_nama:p.ttd4_nama||'',
      ttd4_jabatan:p.ttd4_jabatan||''
    };

    // Compatibility for PJPD saved before V13.
    if(!sig.ttd2_label){
      if(p.menyetujui_nama){
        sig.ttd2_jenis='MENYETUJUI';
        sig.ttd2_label='Menyetujui';
        sig.ttd2_nama=p.menyetujui_nama||'';
        sig.ttd2_jabatan=p.menyetujui_jabatan||'';
      }else if(p.mengetahui1_nama){
        sig.ttd2_jenis='MENGETAHUI';
        sig.ttd2_label='Mengetahui';
        sig.ttd2_nama=p.mengetahui1_nama||'';
        sig.ttd2_jabatan=p.mengetahui1_jabatan||'';
      }

      if(p.mengetahui1_nama && sig.ttd2_nama!==p.mengetahui1_nama){
        sig.ttd3_jenis='MENGETAHUI';
        sig.ttd3_label='Mengetahui';
        sig.ttd3_nama=p.mengetahui1_nama||'';
        sig.ttd3_jabatan=p.mengetahui1_jabatan||'';
      }
      if(p.mengetahui2_nama){
        sig.ttd4_jenis='MENGETAHUI';
        sig.ttd4_label='Mengetahui';
        sig.ttd4_nama=p.mengetahui2_nama||'';
        sig.ttd4_jabatan=p.mengetahui2_jabatan||'';
      }
    }

    const total=Number(p.total_pjpd||0) || (details||[]).reduce((a,d)=>a+Number(d.jumlah||0),0);
    const pdfData={
      ...sig,
      no_pjpd:p.no_pjpd,
      total,
      selisih:Number(p.kurang_lebih_bayar ?? total),
      details:details||[],
      s:sppdForPdf,
      created_at:p.created_at
    };

    await downloadPjpdPdf(JSON.stringify(pdfData));
  }catch(e){
    alert('Gagal membuat ulang PDF PJPD: '+(e?.message||e));
  }finally{
    loading(false);
  }
}
...
window.openDashboardGate=openDashboardGate;
window.closeDashboardGate=closeDashboardGate;
window.verifyDashboardGate=verifyDashboardGate;
window.changeDashboardAccessPassword=changeDashboardAccessPassword;

window.redownloadSppdPdf=redownloadSppdPdf;
window.redownloadPjpdPdf=redownloadPjpdPdf;
